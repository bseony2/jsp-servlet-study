...
<script type="text/javascript">
window.onload= function () {//화면 로딩이 끝나면 totalSum()호출 실행 
	totalSum();
}
...
<button id="deleteAllProduct" onclick="delAll()">선택된 항목 삭제</button>
 총합:<span id="totalSum"></span><!-- 총합을 출력할 부분 -->
</form>
...
<script type="text/javascript">
function totalSum(){//총합을 구하는 - 화면 로딩 완료 후 자동 호출 되어 실행하도록 설정  window.onload사용
	 var totalSum=0;
	 var n= document.querySelectorAll(".sum");//class 모두 선택
	 console.log(n);
	 var result=0;
	for (var i = 0; i < n.length; i++) {
		var num= parseInt(n[i].innerText);//합계하나 얻기 
		console.log(num);
		result+= num;		//총합 구하기 
	}
